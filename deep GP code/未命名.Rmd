...
yp <- higdon(xp)
plot(xp, yp, type = "l", col = 4, xlab = "X", ylab = "Y", main = "4-rank Hermite polinomial and line function")
points(x, y)
...
```{r}
fit2 <- fit_three_layer(year, speed, nmcmc = 10000, verb = FALSE)
fit2 <- predict(fit2, year, lite = FALSE)

plot(fit2,main = "MCMC method three-layer-GP for marathon data")
...
x <- read.csv('./data/x_1.csv')
year <- unlist(x, use.name = FALSE)
y <- read.csv('./data/y_1.csv')
speed <- unlist(y, use.name = FALSE)
```

```{r}

fit2 <- fit_three_layer(x, y, nmcmc = 10000, verb = FALSE)
fit2 <- predict(fit2, x, lite = FALSE)

plot(fit2,main = "MCMC method three-layer-GP for polynomial and line data")

```
```{r}
fit2 <- fit_two_layer(x, y, nmcmc = 10000, verb = FALSE)
fit2 <- predict(fit2, x, lite = FALSE)

plot(fit2,main = "MCMC method two-layer-GP for polynomial and line data")
```
...
plot(xp, yp, type = "l", col = 4, xlab = "X", ylab = "Y", main = "Sgn(x) function")
points(x, y)
```

```{r}
fit1 <- fit_one_layer(x, y, nmcmc = 10000, cov = "exp2", verb = FALSE)

fit1 <- predict(fit1, xp)
plot(fit1,main = 'traditional GPR for sgn(x)')
```
...
plot(fit2,main = 'MCMC method two-layer-GP for sgn(x)')
```
```{r}
fit2 <- fit_three_layer(x, y, nmcmc = 12000, verb = FALSE)

fit2 <- predict(fit2, xp)
plot(fit2,main = 'MCMC method three-layer-GP for sgn(x)')
```

```{r}
fit2 <- fit_one_layer(x, y, nmcmc = 5, verb = FALSE)

fit2 <- predict(fit2, x,lite = FALSE)
plot(fit2,main = 'traditional GP for polymonial and line')
```
